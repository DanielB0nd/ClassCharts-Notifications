...
class ActivityPoint:
    def __repr__(self):
        return '<{} id={!r} point_type={!r} type={!r} score={!r} reason={!r} teacher={!r} lesson={!r}>'.format(self.__class__.__name__, self.id, self.point_type, self.__class__.__name__, self.score, self.reason, self.teacher, self.lesson)
...
class Lesson(BasicLesson):
    ...
    def __repr__(self):
        return '<Lesson name={!r} subject={!r} room={!r}> start={!r} end={!r} period={!r}'.format(self.name, self.subject, self.room, self.start, self.end, self.period)
...
class Timetable:
    ...
    def __repr__(self):
        return "<Timetable date={!r}> start={!r} end={!r}".format(self.date, self.start, self.end)
